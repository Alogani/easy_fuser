...
fn test_zip_fs_mount_and_read() {
    ...
    let mut zip_archive =
        zip::ZipArchive::new(File::open(&zip_path).expect("Failed to open zip file")).unwrap();
    ...
    assert_eq!(
        fs::read_to_string(mount_point.join("file1.txt")).unwrap(),
        "Content of file1\n"
    );
    assert_eq!(
        fs::read_to_string(mount_point.join("file2.txt")).unwrap(),
        "Content of file2\n"
    );
    assert_eq!(
        fs::read_to_string(mount_point.join("subdir/file3.txt")).unwrap(),
        "Content of file3\n"
    );
