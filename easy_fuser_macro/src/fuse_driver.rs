...
fn handle_execution_wrapper(handler_type: HandlerType, block: TokenStream) -> TokenStream {
    match handler_type {
        ...
        HandlerType::Parallel => quote! {
            let reply_executor = self.reply_threadpool.clone();
...
fn handle_reply_wrapper(handler_type: HandlerType, block: TokenStream) -> TokenStream {
    match handler_type {
        HandlerType::Async => quote! {
            todo!("")
        },
        HandlerType::Serial => quote! { { #block } },
        HandlerType::Parallel => quote! {
            reply_executor.execute(move || {
                #block
            })
        },
    }
...
fn expand_macro_placeholders(handler_type: HandlerType, input: TokenStream) -> TokenStream {
    ...
    let arg_idents: Vec<_> = arg_names
        ...
        .map(|arg| {
            match arg.as_ref() {
                // parent and ino shall be converted from u64 to FileREsolver id
                "parent" => format_ident!("parent_id"),
                "ino" => format_ident!("ino_id"),
                _ => format_ident!("{}", arg),
            }
        })
        .collect();
                ...
                if let Some(TokenTree::Ident(ident)) = tokens.next() {
                    ...
                    let replacement = match key.as_str() {
                        ...
                        "ino_id" => {
                            ...
                            quote!(
                                let log_ino: u64 = ino;
                                let ino_id = resolver.resolve_id(ino);
                            )
                        }
                        "parent_id" => {
                            log_ino = true;
                            quote!(
                                let log_ino: u64 = parent;
                                let parent_id = resolver.resolve_id(parent);
                            )
                        }
                        "fh" => quote!(let fh = unsafe { BorrowedFileHandle::from_raw(fh) };),
                        "handle_execution" => {
                            if let Some(TokenTree::Group(group)) = tokens.next() {
                                handle_execution_wrapper(
                                    handler_type,
                                    expand_macro_tokens(
                                        handler_type,
                                        function_name,
                                ...
                                panic!("Expected group after $handle_execution")
                            }
                        }
                        "handle_reply" => {
                            if let Some(TokenTree::Group(group)) = tokens.next() {
                                handle_reply_wrapper(
                                    handler_type,
                                    expand_macro_tokens(
                                        handler_type,
                                        function_name,
                                ...
                                panic!("Expected group after $handle_reply")
                            }
                        }
                        ...
                        unknown => panic!("Unknown $ identifier: {}", unknown),
                    };
                    let replacement = expand_macro_tokens(
                        handler_type,
                        function_name,
                        handler_args,
                        log_ino,
    ...
    match log_ino {
        true => quote! {
            Err(e) => {
                ...
                $handle_reply { reply.error(e.raw_error()) };
                return;
            }
        },
        false => quote! {
...
fn reply_attr() -> TokenStream {
    quote! {
        Ok(file_attr) => {
            ...
            $handle_reply { reply.attr(&ttl.unwrap_or(default_ttl), &fuse_attr) };
        },
    }
}
...
fn reply_ok() -> TokenStream {
    quote! {
        Ok(()) => {
            $handle_reply { reply.ok() }
        },
    }
}
            ...
            #fn_signature {
                ...
                $handle_execution {
                    ...
                    if offset < 0 {
                        ...
                        $handle_reply { reply.error(ErrorKind::InvalidArgument.into()) };
                        return;
                    }
                ...
                fn bmap(&mut self, req: &Request<'_>, ino: u64, blocksize: u32, idx: u64, reply: ReplyBmap) {
                    ...
                    $handle_execution {
                        ...
                        match handler.bmap($handler_args)$await {
                            Ok(block) => $handle_reply { reply.bmap(block) },
                            $warn_error
                        }
                    };
                }
            ...
            fn copy_file_range(
                ...
                $handle_execution {
                    ...
                    match handler.copy_file_range($handler_args)$await {
                        Ok(bytes_written) => $handle_reply { reply.written(bytes_written) },
                        $warn_error
                    }
                };
            }
    ...
    result.push(expand_macro_placeholders(
        ...
        quote! {
            fn forget(&mut self, req: &Request, ino: u64, nlookup: u64) {
                $req
                let ino_id = self.resolver.resolve_id(ino);
                self.handler.forget(&req, ino_id, nlookup);
                ...
                fn getlk(
                    ...
                    $handle_execution {
                        ...
                        match handler.getlk(&req, ino_id, fh, lock_owner, lock_info)$await {
                            Ok(lock_info) => $handle_reply { reply.locked(lock_info.start,
                                lock_info.end,
            ...
            fn ioctl(
                ...
                $handle_execution {
                    ...
                    match handler.ioctl($handler_args)$await {
                        Ok((result, data)) => $handle_reply { reply.ioctl(result, &data) },
                        $warn_error
                    };
                };
            }
            ...
            fn lseek(
                ...
                $handle_execution {
                    ...
                    match handler.lseek(
                        ...
                        Ok(new_offset) => $handle_reply { reply.offset(new_offset) },
                        $warn_error
                    };
                }
            }
            ...
            fn open(&mut self, req: &Request, ino: u64, flags: i32, reply: ReplyOpen) {
                ...
                $handle_execution {
                    ...
                    match handler.open($handler_args)$await {
                        Ok((file_handle, response_flags)) => $handle_reply { reply.opened(file_handle.as_raw(), response_flags.bits()) },
        ...
        quote! {
            fn read(
                ...
                $handle_execution {
                    $ino_id
                    $fh
                    let offset = seek_from_raw(Some(match i32::try_from(offset) {
                        Ok(offset) => offset,
                        Err(_) => {
                            $handle_reply { reply.error(ErrorKind::InputOutputError.into()) };
                            return;
                        }
                    }), 0);
                    let flags = FUSEOpenFlags::from_bits_retain(flags);
                    match handler.read($handler_args)$await {
                        Ok(data_reply) => $handle_reply { reply.data(&data_reply) },
                        $warn_error
                    };
                };
            }
            ...
            fn readlink(&mut self, req: &Request, ino: u64, reply: ReplyData) {
                ...
                $handle_execution {
                    ...
                    match handler.readlink($handler_args)$await {
                        Ok(link) => $handle_reply { reply.data(&link) },
                        $warn_error
                    };
                };
            }
    ...
    result.push(expand_macro_placeholders(
        ...
        quote! {
            fn rename(
                ...
                $handle_execution {
                    let parent_id = resolver.resolve_id(parent);
                    let newparent_id = resolver.resolve_id(newparent);
            ...
            fn statfs(&mut self, req: &Request, ino: u64, reply: ReplyStatfs) {
                ...
                $handle_execution {
                    ...
                    match handler.statfs($handler_args)$await {
                        Ok(statfs) => $handle_reply { reply.statfs(
                            statfs.total_blocks,
                            statfs.free_blocks,
...
fn get_dependencies() -> proc_macro2::TokenStream {
    quote! {
        ...
        use fuser::{
            self, Filesystem, KernelConfig, ReplyAttr, ReplyBmap, ReplyCreate, ReplyData, ReplyDirectory,
